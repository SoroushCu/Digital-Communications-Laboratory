...
bw = 100e3;
t = 0:n_fltr-2;
w = 3/2*n_fltr/(n_fltr^2 - 1)*(1 - ((t - (n_fltr/2 - 1))/(n_fltr/2)).^2);
...
% Receiver Parameters
rx_fc = 98e6; % Set Receiver Center Frequency
% (AD9363: 325-3800MHz) (AD9364: 70-6000MHz)
fs=20e6;
...
while toc < stop_time
    rx_data = rx();
    rx_data_dec = decimate(rx_data, floor(fs/bw), 512, 'fir');
    x = rx_data_dec.*conj(circshift(rx_data_dec, 1));
    demod_data = movmean(atan2(imag(x), real(x))*fs/(2*pi), 100);
    sound(conv(demod_data, w, 'valid'), fs/round(fs/bw));
end
